...
void main()
{
	...
	if(dist < 2 && dist > 1.8)
	{
		...
		//if(mod(dot(normalize(VertexPosition), normalize(rotatedTimeVec)), 0.2) > 0.1)
		//{
		...
		//}
		out_color = vec4(1, 1, 1, 1);
		float effectPercentage = mod((dot(normalize(VertexPosition), normalize(rotatedTimeVec)) + 1)/2, 0.2);
		if(effectPercentage > 0.05 && effectPercentage < 0.15)
		{
